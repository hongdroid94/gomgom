...
import { mapperUserEntityToUserDto } from '../mapper';
...
async function fetchUserRegistered(): Promise<boolean | UserDto> {
    ...
    return mapperUserEntityToUserDto(await authApi.findUserByEmailAndLoginType(
        data.user.email,
        data.user.app_metadata.provider === 'email' ? LoginType.EMAIL : LoginType.GOOGLE
    ));
}
